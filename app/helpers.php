...
function client_friendly_text($value): string
{
    return strtr((string) $value, [
        'Дизайн и 3D' => 'Дизайн и трёхмерная визуализация (3D)',
        '3D визуализация' => 'трёхмерная визуализация (3D)',
        '3D-визуализация' => 'Трёхмерная визуализация (3D)',
        '3D-модель' => 'трёхмерная модель (3D)',
        '3D-тур' => 'виртуальный трёхмерный тур (3D)',
        '3D-картинки' => 'трёхмерной картинки (3D)',
        '3D-резьба' => 'трёхмерная резьба (3D)',
        'Фотореалистичная 3D' => 'Фотореалистичная трёхмерная визуализация (3D)',
        'показываем 3D' => 'показываем трёхмерную модель (3D)',
        'по 3D.' => 'по трёхмерной визуализации (3D).',
        'Станки ЧПУ' => 'Станки с числовым программным управлением (ЧПУ)',
        'станках с ЧПУ' => 'станках с числовым программным управлением (ЧПУ)',
        'на станках ЧПУ' => 'на станках с числовым программным управлением (ЧПУ)',
        'на ЧПУ' => 'на станках с числовым программным управлением (ЧПУ)',
        'после ЧПУ' => 'после обработки на станке с числовым программным управлением (ЧПУ)',
        'LED-ленты' => 'светодиодные ленты (LED)',
        'LED-лента' => 'светодиодная лента (LED)',
        'LED-подсветка' => 'светодиодная подсветка (LED)',
        'Подсветка LED' => 'Светодиодная подсветка (LED)',
        'LED-профиль' => 'профиль для светодиодной подсветки (LED)',
        'IP65' => 'со степенью защиты IP65',
        'УФ-излучения' => 'ультрафиолетового излучения (УФ)',
        'УФ-фильтром' => 'фильтром от ультрафиолета (УФ)',
        'УФ-фильтры' => 'фильтры от ультрафиолета (УФ)',
        'Защита от УФ' => 'Защита от ультрафиолета (УФ)',
        'ДВП' => 'древесноволокнистой плиты (ДВП)',
        'ДПК' => 'древесно-полимерного композита (ДПК)',
        'ПВХ' => 'поливинилхлорида (ПВХ)',
        'ДСП' => 'древесно-стружечной плиты (ДСП)',
        'RAL' => 'RAL (международный каталог цветов)',
        'СанПиН' => 'санитарным нормам и правилам (СанПиН)',
        'СП ' => 'строительным правилам (СП) ',
        'RGB' => 'RGB (цветная подсветка)',
        'ГОСТ' => 'государственному стандарту (ГОСТ)',
        'VOC' => 'летучие органические соединения (VOC)',
        'CAD/CAM' => 'системах автоматизированного проектирования и производства (CAD/CAM)',
    ]);
}

function h($value): string
{
    return htmlspecialchars(client_friendly_text($value), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
}
...
function social_link_html(array $social, string $linkClass, string $iconClass): string
{
    ...
    return '<a class="' . h($linkClass) . '" href="' . h($social['href']) . '" aria-label="' . h($social['label']) . '"'
        ...
        . '><img class="social-media-icon social-media-icon--' . h($social['name']) . ' ' . h($iconClass) . '" src="' . h(asset_url('icons/' . $social['name'] . '.svg')) . '?v=20260723-social-fix" alt="" width="36" height="36"></a>';
}
...
function render_layout(array $site, string $title, string $content, array $seo = []): void
{
    ...
    echo '<link rel="stylesheet" href="/assets/css/site.css?v=20260724-product-gallery">';
...
function service_gallery_images(array $page): array
{
    $images = [
        ...
        'dekor' => ['decor/designer-decor-1.jpg', 'decor/designer-decor-2.jpg', 'decor/designer-decor-3.jpg', 'decor/designer-decor-4.jpg', 'decor/designer-decor-5.jpg'],
        'dekor-fasadov-ograzhdenij-balkonov' => ['decor/decor-1.webp', 'decor/decor-2.webp', 'decor/decor-3.webp', 'decor/decor-4.webp', 'decor/decor-5.webp'],
    ...
    foreach ($images[$slug] ?? [] as $file) {
        $gallery[] = ['url' => str_starts_with($file, '/') ? $file : asset_url('images/Projects/' . $file), 'alt' => (string) ($page['title'] ?? '')];
    }
